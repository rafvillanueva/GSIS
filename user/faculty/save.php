...
if(isset($_POST['faci_d'])){
	...
	$select_abc = mysqli_query($conn, "SELECT SUM(*) as absent FROM tbl_scores WHERE (Fac_id = 'faci_d' AND Stud_id = '$stud_id'");
	$row = mysqli_fetch_array($select_abc);
	$total_absent = $row['absent'] + $absent;
	if($row['absent'] >= $total_absent){
		...
		$savez = mysqli_query($conn, "INSERT INTO tbl_scores(Fac_id,Stud_id,SubjectCode,Year,Semester,Grade_Components,Period,Score,Total_score,Grade, Status,Date_edit,Section) VALUES('$faci_d','$stud_id','$sub_id','$Acad','$sem','Unexcused absences incurred','$Period','0','0','0','Sent','$date','$sectionx')");
	}else{
		$save = mysqli_query($conn, "INSERT INTO tbl_scores(Fac_id,Stud_id,SubjectCode,Year,Semester,Grade_Components,Period,Score,Total_score,Grade, Status,Date_edit,Section) VALUES('$faci_d','$stud_id','$sub_id','$Acad','$sem','$Quiz','$Period','$Score','$Total_score','$Grade','Sent','$date','$sectionx')");
		$savez = mysqli_query($conn, "INSERT INTO tbl_scores(Fac_id,Stud_id,SubjectCode,Year,Semester,Grade_Components,Period,Score,Total_score,Grade, Status,Date_edit,Section) VALUES('$faci_d','$stud_id','$sub_id','$Acad','$sem','Unexcused absences incurred','$Period','$absent','0','0','Sent','$date','$sectionx')");
	}
...
/*
	...
	<script type="text/javascript">
		...
		document.getElementById("'.$eabc1.'").disabled = false;
...
if(isset($_POST['ufaci_d'])){
	...
	$update_abc = mysqli_query($conn, "UPDATE tbl_scores SET Score = '$absent', Grade = '$Grade', Date_edit = '$date', Total_score = '$Total_score' WHERE Fac_id = '$faci_d' AND Stud_id = '$stud_id' AND SubjectCode = '$sub_id' AND Year = '$Acad' AND Semester = '$sem' AND Grade_Components = 'Unexcused absences incurred' AND Period = '$Period'");
