  ...
  foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
  {
   ...
   for($row=13; $row<=$highestRow; $row++)
   {
    ...
    if(!empty($name)){
      ...
      $section = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(1, 10)->getValue());
      #score names
      ...
      $absent = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(2, $row)->getValue());
      ...
      $q1_over = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(3, 13)->getValue());
      $q1_grade = ($q1 / $q1_over) * 50 + 50;
      if($worksheet->getCellByColumnAndRow(3, $row)->getValue()){
        $components = mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(3, 12)->getValue());
      }
      $vr1 = mysqli_query($conn, "SELECT * FROM tbl_scores WHERE Stud_id = '$std_id' AND Grade_Components = '$components' AND Year = '$year' AND Semester = '$sem' AND Period = '$period' AND Fac_id = '$faculty_id'");
      if(mysqli_num_rows($vr1) == 0){
        $q1_query = "
    ...
    $output .= '<td>' . $num++ .'</td>';
    $output .= '<td>' . $name .'</td>';
    $output .= '<td><center>' . $exist_abent .'</center></td>';
    $output .= '<td><center>' . $exist_q1 .'</center></td>';
